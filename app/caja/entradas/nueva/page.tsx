...
import { Loader2, PlusCircle, ArrowLeft, X, Trash2 } from "lucide-react"
...
export default function NuevaEntradaCajaPage() {
  ...
  const agregarMovimiento = () => {
    ...
    const nuevoMovimiento: MovimientoEntrada = {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const entradasData = movimientosAEnviar.map(movimiento => ({
        tipo: 'ENTRADA',
      ...
      console.log("Nuevas entradas de caja:", entradasData)

      const totalMovimientos = entradasData.length
      const totalMonto = entradasData.reduce((sum, entrada) => sum + entrada.monto, 0)

      toast({
        title: "Entradas Registradas",
        description: `${totalMovimientos} entrada(s) por un total de Q${totalMonto.toLocaleString("es-GT", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} registrada(s) exitosamente`,
      })
      ...
      toast({
        title: "Error",
        description: error.message || "No se pudieron registrar las entradas",
        variant: "destructive",
      })
    } finally {
      setSubmitting(false)
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Nueva Entrada de Caja</h1>
          <p className="text-muted-foreground">Registra una nueva entrada de efectivo al sistema de caja.</p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Permite registrar múltiples entradas en una sola operación
              </p>
            </div>
            <Switch
        ...
        <Card>
          ...
          <CardContent className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Textarea
                id="descripcion"
                placeholder="Describe el motivo de la entrada (ej: Venta de productos, cobro de factura, etc.)"
            ...
            <div className="grid gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Referencia opcional para identificar la transacción (número de factura, recibo, etc.)
              </p>
            </div>
            ...
            {modoMultiple ? (
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center text-lg font-bold pt-2 border-t">
                  ...
                  <span className="text-green-600">{formatMonto(totalMovimientos)}</span>
                </div>
                <div className="flex justify-between text-lg font-bold">
                  <span>Tipo de Movimiento:</span>
                  <span className="text-green-600">ENTRADAS MÚLTIPLES</span>
                </div>
              </div>
            ) : (
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? (
              ...
              <>
                <PlusCircle className="mr-2 h-4 w-4" />
                {modoMultiple ? `Registrar ${movimientos.length} Entradas` : "Registrar Entrada"}
              </>
            )}
          </Button>
        </div>
