...
/**
 * GET /api/piedrinera/camiones/[id]
 * Obtiene un camión específico desde Django
 */
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${DJANGO_API_URL}/api/piedrinera/camiones/${id}/`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al obtener camión' },
    ...
    return NextResponse.json(
      { error: 'Error al obtener el camión' },
      { status: 500 }
    )
  }
}
...
/**
 * PUT /api/piedrinera/camiones/[id]
 * Actualiza un camión en Django
 */
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al actualizar camión' },
        { status: response.status }
      )
    }
...
/**
 * DELETE /api/piedrinera/camiones/[id]
 * Elimina un camión en Django
 */
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      return NextResponse.json(
        errorData || { error: 'Error al eliminar camión' },
        { status: response.status }
      )
    }

    return NextResponse.json({ message: 'Camión eliminado exitosamente' }, { status: 204 })
  } catch (error) {
    console.error('Error al eliminar camión:', error)
    return NextResponse.json(
      { error: 'Error al eliminar el camión' },
      { status: 500 }
    )
  }
}
