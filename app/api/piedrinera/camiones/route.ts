...
/**
 * GET /api/piedrinera/camiones
 * Obtiene la lista de camiones desde Django
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const djangoUrl = new URL(`${DJANGO_API_URL}/api/piedrinera/camiones/`)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al obtener camiones' },
    ...
    return NextResponse.json(
      { error: 'Error al obtener los camiones' },
      { status: 500 }
    )
  }
}
...
/**
 * POST /api/piedrinera/camiones
 * Crea un nuevo camión en Django
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Hacer proxy a Django
    const response = await fetch(`${DJANGO_API_URL}/api/piedrinera/camiones/`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al crear camión' },
        { status: response.status }
      )
    }
