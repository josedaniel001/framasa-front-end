 ...
 * Obtiene la lista de empleados desde Django
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const djangoUrl = new URL(`${DJANGO_API_URL}/api/planillas/empleados/`)
...
/**
 * POST /api/planillas/empleados
 * Crea un nuevo empleado en Django
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Hacer proxy a Django
    const response = await fetch(`${DJANGO_API_URL}/api/planillas/empleados/`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al crear empleado' },
        { status: response.status }
      )
    }
