  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al convertir cotización a factura' },
    ...
    return NextResponse.json(
      { error: 'Error al convertir la cotización a factura', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
