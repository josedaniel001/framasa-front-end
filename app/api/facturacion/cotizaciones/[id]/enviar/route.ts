...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${DJANGO_API_URL}/api/facturacion/cotizaciones/${params.id}/enviar/`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        errorData || { error: 'Error al enviar cotización' },
    ...
    return NextResponse.json(
      { error: 'Error al enviar la cotización', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
