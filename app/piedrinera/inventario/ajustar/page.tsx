  ...
  const handleAdjust = (e: React.FormEvent) => {
    ...
    if (!itemSeleccionado || cantidadAjuste <= 0 || !razonAjuste) {
      toast({
        ...
        description: "Por favor, selecciona un agregado, ingresa una cantidad válida y una razón para el ajuste.",
        variant: "destructive",
      })
      return
    }
    ...
    if (!itemToAdjust) {
      toast({
        title: "Error",
        description: "Agregado de inventario no encontrado.",
        variant: "destructive",
      })
      return
    }

    let newQuantity = itemToAdjust.cantidadMetrosCubicos
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="grid gap-2">
              <Label htmlFor="item">Agregado de Inventario</Label>
                ...
                <SelectContent>
                  {inventarioItems.map((item) => (
                    <SelectItem key={item.id} value={item.id}>
                      {item.nombreAgregado} ({item.codigo}) - Stock: {item.cantidadMetrosCubicos}m³
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
